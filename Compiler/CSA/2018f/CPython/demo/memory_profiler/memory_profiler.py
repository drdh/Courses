...
from ctypes import *
mylib = CDLL("../lib/libwrapper.so")

enter_function = mylib.enter_function
enter_function.argtypes = [c_char_p, c_char_p, c_byte]
enter_function.restype = c_int

exit_function = mylib.exit_function
exit_function.argtypes = []
exit_function.restype = c_int
...
class CGNode(NodeMixin):  
    def __init__(self, func=None, parent=None):
        ...
        self.name = 'function:%s\ncall_num:%d\nmax_runtime:%fms\nmin_runtime:%fms\ntotal_runtime:%fms' % (self.func,self.call_num,self.max_runtime,self.min_runtime,self.total_runtime)
        self.parent = parent
        self.begintime = 0
    def updatename(self):
        self.name = 'function:%s\ncall_num:%d\nmax_runtime:%fms\nmin_runtime:%fms\ntotal_runtime:%fms' % (self.func,self.call_num,self.max_runtime,self.min_runtime,self.total_runtime)
    def setfunc(self, func):
        self.func = func
        self.updatename()
    def callbegin(self, begintime):
        self.call_num = self.call_num + 1
        self.begintime = begintime
    def callend(self, endtime):
        if self.begintime > 0:
            runtime = endtime - self.begintime
            self.begintime = 0
        else:
            raise Exception("call process reach the end but begintime is zero")
        if runtime < self.min_runtime or self.min_runtime < 0.0000001:
            self.min_runtime = runtime
        if runtime > self.max_runtime:
            self.max_runtime = runtime
        self.total_runtime = self.total_runtime + runtime
        self.updatename()
    def empty(self):
...
#modified end
    ...
    def __init__(self, **kw):
        ...
        #modified begin
        self.callanalysis = kw.get('callanalysis', False)
        self.flag = 0
        #modified end
        self.prevlines = []
    ...
    #modified begin
    global flag
    def call_analysis(self, frame, event, arg):
        global pre
        #print(event)
        if event == 'call':
            callee = frame.f_code.co_name
            caller = frame.f_back.f_code.co_name
            if callee == "disable" or callee == "disable_by_count":
                pass
            else:
                if frame.f_locals.__contains__('self'):
                    mangle_callee = "python_" + frame.f_globals['__name__']+ "_" + frame.f_locals['self'].__class__.__name__ + "_" + callee
                else:
                    mangle_callee = "python_" + frame.f_globals['__name__']+ "_" + "global"+ "_" + callee
                c_callee = c_char_p(callee.encode('utf-8'))
                c_mangle_callee = c_char_p(mangle_callee.encode('utf-8'))
                language = c_byte(2)
                global flag
                if flag == 0:
                    flag = 1
                    ...
                    load()
                    #print("load")
                enter_function(c_mangle_callee,c_callee,language)
                save()
            #if callee == 'disable' or callee == 'disable_by_count' or caller == 'disable' or caller == 'disable_by_count':
            '''if False:
                pass
            elif root.empty() == True:
                root.setfunc(caller)
                _child = CGNode(callee, parent = root)
                _child.callbegin(time.time() * 1000)
                pre = _child
            else:
                if(pre.func == 'disable'):
                ...
                if(caller != pre.func):
                    print(caller)
                    print(pre.func)
                    raise Exception('Call process trace error')
                if False : #pre.func == pre.parent.func
                    callee_node = pre
                    callee_node.callbegin(time.time() * 1000)
                else:
                    callee_node = findall(root,filter_=lambda node: node and node.func == callee and node.parent == pre)
                    if len(callee_node) == 0:
                        callee_node = CGNode(callee, parent = pre)
                        callee_node.callbegin(time.time() * 1000)
                        #global pre
                        pre = callee_node
                    else:
                        callee_node[0].callbegin(time.time() * 1000)
                    #global pre
                        pre = callee_node[0]'''
        if event == 'return':
            load()
            ...
            if callee != caller:
                ...
                if self.flag == 0:
                    pre = pre.parent
                    self.flag = 1
            #print(callee,caller,pre.func)
            pre = pre.parent
            #if caller == 'f' or callee == 'disable' or callee == 'disable_by_count' or caller == 'disable' or caller == 'disable_by_count':
            if False:
                pass
            else:
                callee_n = findall(root,filter_=lambda node: node and node.func == callee and node.parent == pre)
                if len(callee_n) > 1:
                    for cnode in callee_n:
                        if cnode.begintime > 0:
                            callee_node = cnode
                            break
                callee_node = callee_n
                if len(callee_node) == 0 or len(callee_node) > 1:
                    raise Exception("more than one callees are found!")
                callee_node[0].callend(time.time() * 1000)
                #global pre'''
        if self._original_trace_function is not None:
            self._original_trace_function(frame, event, arg)
        return self.call_analysis
    #modified end
    ...
    def enable(self):
        ...
        #modified begin
        elif self.callanalysis is True:
            sys.settrace(self.call_analysis)
        #modified end
        else:
...
#modified begin
'''
def delete_irrelevant_node():
    extranode = findall(root,filter_=lambda node : node.func == 'enable_by_count')
    for nodes in extranode:
        nodes.parent = None
    extranode = findall(root,filter_=lambda node : node.func == 'disable_by_count')
    for nodes in extranode:
        nodes.parent = None
    extranode = findall(root,filter_=lambda node : node.parent and node.parent.func == 'f')
    for nodes in extranode:
        if nodes.parent != root:
            p = nodes.parent
            nodes.parent = nodes.parent.parent
            p.parent = None
    exctranode = findall(root,filter_=lambda node : node != root and node.func == 'f')
    for nodes in extranode:
        nodes.parent = None
    root.setfunc('root')
    root.name = 'root'
...
def handle_recursion():
    extranode = findall(root,filter_=lambda node : node.parent and node.parent.func == node.func)
    if len(extranode) > 0:


def output_callgraph():
    extranode = findall(root,filter_=lambda node : node.func == 'disable_by_count')
    extranode[0].parent = None
    root.setfunc('root')
    root.name = 'root' 
    delete_irrelevant_node()
    DotExporter(root).to_picture("callgraph.png")
#modified end'''
...
if __name__ == '__main__':
    ...
    #modified begin
    parser.add_argument('--callanalysis', dest='callanalysis', default=False, action='store_true',
        help='print the call graph')
    #modified end
    ...
    if args.timestamp:
        ...
        prof = LineProfiler(max_mem=args.max_mem, backend=_backend, callanalysis = args.callanalysis)
