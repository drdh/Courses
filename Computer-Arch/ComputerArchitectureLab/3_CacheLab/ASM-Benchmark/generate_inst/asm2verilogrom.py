...
if len(sys.argv) != 3:
    ...
    res = os.system( '%sriscv32-elf-as.exe %s            -o compile_tmp.o   -march=rv32i' % (RISCV_TOOLCHAIN_PATH, INPUT) )
    if res != 0:
        print('\n    Assembling Error!')
        sys.exit()
    os.system( '%sriscv32-elf-ld.exe compile_tmp.o -o compile_tmp.om'               % (RISCV_TOOLCHAIN_PATH       ) )
    os.system( 'del compile_tmp.o'   )
    os.system( '%sriscv32-elf-objcopy.exe -O binary compile_tmp.om compile_tmp.bin' % (RISCV_TOOLCHAIN_PATH,      ) )
