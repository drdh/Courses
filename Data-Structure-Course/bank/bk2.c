...
int Bank(int CloseTime,int *Py_time,int *Py_q1,int *Py_q2,int *Py_Total,int *Py_TotalNum,int *Py_TotalTime)
{
    OpenForDay();
  //  PrintL(ev);
  //  PrintQ(q[1]);
  //  PrintQ(q[2]);
    ...
    //    printf("total=%d\n",Total);    
        ...
        count++;
        Py_time[count]=event.time;
        Py_q1[count]=QueueLength(q[1]);
        Py_q2[count]=QueueLength(q[2]);
        Py_Total[count]=Total;
        Py_TotalNum[count]=TotalNum;
        Py_TotalTime[count]=TotalTime;
