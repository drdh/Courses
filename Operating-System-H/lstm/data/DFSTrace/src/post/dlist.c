...
static char *rcsid = "$Header: /home/cvs/root/DFSTrace/src/post/dlist.c,v 1.2 1998/10/13 21:14:31 tmkr Exp $";
*/
#endif _BLURB_
...
void dlist::print(int fd) {
    ...
    char buf[80];
    sprintf(buf, "%#08lx : Default Dlist : count = %d\n",
	     (long)this, cnt);
...
void dlink::print(int fd) {
    char buf[80];
    sprintf(buf, "%#08lx : Default Dlink : prev = %lx, next = %lx\n",
	     (long)this, (long)prev, (long)next);
    write(fd, buf, strlen(buf));
}
