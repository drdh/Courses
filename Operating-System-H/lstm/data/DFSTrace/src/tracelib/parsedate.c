/**
 * $Id: parsedate.c,v 1.3 1998/10/13 21:14:32 tmkr Exp $
 *
 *   This code originated from a CMU libcs  routine with a yacc
 * file parsedate.y.  This is the c code automatically generated
 * and then cleaned up to remove all of the  compiler warnings
 *
 *
 **********************************************************************
 * HISTORY
 * $Log: parsedate.c,v $
 * Revision 1.3  1998/10/13 21:14:32  tmkr
 * updated make files and some sources to improve multi-platform compiles
 *
 ...
 *
 * Revision 1.1.1.1  1998/09/29 18:39:19  tmkr
...
static int pcount;
static int result;
static int yylval, yyval;

/**
 * forward referencing for local functions added by hand--tmk
 */
static int yyparse();
static  void setrep();
static  void time_def();
static  void incr ();
static void yyerror();
static int nextlex ();
static   void peeper ();
...
int parsedate (str, tmp, settm, select, err)
     char *str;
     struct tm *tmp;
...
static int prelex() {
    register int ret= 0;	        /* temp for previous delimiter */
...
static  void constrain (con, date, past, repeat)
     int con, date;
     int past, repeat;
{ register int n;
 register int did;
...
static  void time_def (tm, currtm)
     struct tm *tm, *currtm;
...
static void incryear (ndtm, years)
     int ndtm;
     int years;
{ setrep (&dtm[ndtm], RTM);
...
static void incrmonth (ndtm, months)
     int ndtm;
     int months;
{ int inc;
 inc = months > 0 ? 1 : -1;
...
#endif
static int yyv[YYMAXDEPTH]; /* where the values are stored */
