...
static char *rcsid = "$Header: /home/cvs/root/DFSTrace/src/tracelib/v3.c,v 1.6 1998/10/23 03:37:58 tmkr Exp $";
*/
#endif _BLURB_
...
/*
 * CheckV3FilePreamble -- returns whether or not the preamble makes sense.
 */
int CheckV3FilePreamble(preamblePtr)
dfs_file_preamble_v3_t *preamblePtr;
{
	int truth = 0;
	...
	if (((preamblePtr->hostAddress >> 24) == 128) &&
	    ...
	    (preamblePtr->bootTime < V3_END) &&
	    (preamblePtr->bootTime > V3_START) &&
	    (preamblePtr->agentBirthTime < V3_END) &&
	    (preamblePtr->agentBirthTime > V3_START) &&
	    ((preamblePtr->kernelVersion >> 8) == 3) &&
	    ((int)(preamblePtr->kernelVersion & 0x00ff)
	           <= KERNELVERSIONMAX_V3) &&
	    ((((preamblePtr->agentVersion >> 8) == 3) &&
	      ((int)(preamblePtr->agentVersion & 0x00ff)
	          <= AGENTVERSIONMAX_V3)) ||
	     ((preamblePtr->agentVersion >> 8) == 2)) && /* allow this */
	    ((((preamblePtr->collectorVersion >> 8) == 3) &&
	      ((int)(preamblePtr->collectorVersion & 0x00ff)
	           <= COLLECTORVERSIONMAX_V3)) ||
...
/*
 * V3FilePreamble -- interprets a buffer of DECODE_AMOUNT bytes
 * and decides whether or not it's a valid version 1 file preamble.
 * Returns 1 if so, 0 if not.
 */
int V3FilePreamble(buf)
char *buf;
{
	dfs_file_preamble_v3_t preamble;
	int offset = 0;
	...
	return(CheckV3FilePreamble(&preamble));
}

/*
 * UnpackV3FilePreamble -- reads preamble written by collection server from file fp
 ...
 */
int UnpackV3FilePreamble(tfPtr)
trace_file_t *tfPtr;
{
	char *buf;
	int offset = 0;
	...
	buf = (char *) malloc(sizeof(dfs_file_preamble_v3_t));
	if (fread(buf, 1, sizeof(dfs_file_preamble_v3_t), tfPtr->fp) !=
	    sizeof(dfs_file_preamble_v3_t)) {
		free(buf);
		return(TRACE_FILEREADERROR);
	}

	tfPtr->preamblePtr = (char *) malloc(sizeof(dfs_file_preamble_v3_t));
	tfPtr->chunkPreamblePtr = (char *) malloc(sizeof(dfs_chunk_preamble_v3_t));
	tfPtr->chunk = (char *) malloc(DFS_TRACE_CHUNK_SIZE_V3);

	preamblePtr = (dfs_file_preamble_v3_t *) tfPtr->preamblePtr;
	...
	tfPtr->traceStats.totalBytes += sizeof(dfs_file_preamble_v3_t);
	free(buf);
 ...
 * PrintV3FilePreamble
 */
void PrintV3FilePreamble(tfPtr)
trace_file_t *tfPtr;
{
	dfs_file_preamble_v3_t *preamblePtr = (dfs_file_preamble_v3_t *) tfPtr->preamblePtr;
	char *kv, *av, *cv, bt[25];
...
/*
 * UnpackV3ChunkPreamble -- reads chunk preamble written by agent from file
 ...
 */
int UnpackV3ChunkPreamble(tfPtr)
trace_file_t *tfPtr;
{
	char *buf;
	int offset = 0;
	int oldlost;
	dfs_chunk_preamble_v3_t *preamblePtr;

	buf = (char *) malloc(sizeof(dfs_chunk_preamble_v3_t));
	if (fread(buf, 1, sizeof(dfs_chunk_preamble_v3_t), tfPtr->fp) !=
	    sizeof(dfs_chunk_preamble_v3_t)) {
		free(buf);
		return(TRACE_FILEREADERROR);
	}
	preamblePtr = (dfs_chunk_preamble_v3_t *) tfPtr->chunkPreamblePtr;
	...
	tfPtr->traceStats.totalBytes += sizeof(dfs_chunk_preamble_v3_t);
	free(buf);
	return(TRACE_SUCCESS);
}

/*
 * UnpackV3Chunk -- unpacks the next chunk into tfPtr->chunk.
 * returns TRACE_SUCCESS if everything was ok, otherwise
 * returns TRACE_FILEREADERROR.
 */
int UnpackV3Chunk(tfPtr)
trace_file_t *tfPtr;
{
	if (debug)
		printf("Reading new chunk at location %ld\n",tfPtr->traceStats.totalBytes);

	if (fread(tfPtr->chunk, 1, DFS_TRACE_CHUNK_SIZE_V3, tfPtr->fp) !=
	    DFS_TRACE_CHUNK_SIZE_V3) {
		return(TRACE_FILEREADERROR);
	}
	tfPtr->chunkOffset = 0;
	tfPtr->traceStats.totalBytes += DFS_TRACE_CHUNK_SIZE_V3;
	return(TRACE_SUCCESS);
}

/*
 * UnpackFirstV3RecordTime -- picks out the time of the first trace
 * record from the decode buffer, puts it in the statistics block.
 */
void UnpackFirstV3RecordTime(tfPtr, buf)
trace_file_t *tfPtr;
char *buf;
{
	int offset = 40;
...
/*
 * UnpackV3Record -- unpacks the record at the current chunk offset,
 * and returns a record of the appropriate size and type in recordPtrPtr,
 * or NULL if there was an error.
 */
void UnpackV3Record(tfPtr, recordPtrPtr)
trace_file_t *tfPtr;
dfs_header_t **recordPtrPtr;
{
	...
	*recordPtrPtr = NULL;
	...
	if ((tfPtr->chunkOffset == -1) ||
	    (tfPtr->chunkOffset >= DFS_TRACE_CHUNK_SIZE_V3) ||   /* a problem */
	    (*((short *) &tfPtr->chunk[tfPtr->chunkOffset]) == 0)) {
		/* get a new chunk. preamble first */
		if (UnpackV3ChunkPreamble(tfPtr)) {
			if (verbose)
	...
	if (curOffset + length >= DFS_TRACE_CHUNK_SIZE_V3) {
	...
	default:
		printf("Bogus bytes! chunk at %ld, offset %d, opcode = 0x%x, flags = 0x%x\n",
		       tfPtr->traceStats.totalBytes - DFS_TRACE_CHUNK_SIZE_V3, tfPtr->chunkOffset,
		       tfPtr->chunk[tfPtr->chunkOffset],
	...
	if (timercmp(&(*recordPtrPtr)->time, &tfPtr->traceStats.lastTraceRecordTime, <)) {
		printf("Warning: Decreasing timestamp. Chunk %ld, offset %d, opcode %s.\n\tLast timestamp was %s",
		       tfPtr->traceStats.totalBytes - DFS_TRACE_CHUNK_SIZE_V3, curOffset,
