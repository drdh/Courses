...
{
  Thread *child=kernel->getThreadByID(tid);
  if(child)
  {
  Semaphore* sema=new Semaphore("join semaphore",0);
  joinSemMap_insert(tid, sema);
  sema->P();
  joinSemMap_remove(tid);
  }
}
...
Thread::Thread(char *threadName)
{
    ...
    priority = CREATE_PRIORITY + random() % 50;
...
void Thread::Finish()
{
    ...
    { //begin: 如果需要，请把实验二的代码请写在这里++++++++++++++++++++++++++++++++++++++++
        // 如果不需要，请忽视
    Semaphore *sema=parent->joinSemMap_getSemByID(getTid());
    if(sema)
	sema->V();
    } //end: +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
