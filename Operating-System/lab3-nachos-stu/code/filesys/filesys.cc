...
bool FileSystem::Create(char *name, int initialSize)
{
    ...
    //   例如，name为“folder1/folder2/file1“,则需要从根目录出发，通过其下的folder1到达folder2
    //   ，然后在folder2目录下创建文件file1，若folder1或folder2不存在，则失败返回FALSE。

    int  i;
    for (i = 0; i < folderStrVec.size() - 1; i++) {
	sector=currentDirectory->Find((char*)folderStrVec[i].c_str());
	if(sector==-1)
	  return FALSE;
...
//   将name包含的目录名解析出来，然后从根目录开始跳转定位到当前目录。
//   若name中包含的目录不存在，则认为失败并返回FALSE。
//   最后在当前目录下创建相应的目录，具体过程可参考NachOS的根目录创建过程。
//   例如，name为“folder1/folder2/folder3“,则需要从根目录出发，通过其下的folder1到达folder2
//   ，然后在folder2目录下创建名为folder3的文件（目录就是一个文件），若folder1或folder2不存在，则失败返回FALSE。
...
bool FileSystem::CreateFolder(char *name)
{
   ...
   int i;
    ...
    else
    {
		...
		Directory *directory=new Directory(NumDirEntries);
                hdr->WriteBack(sector);
		OpenFile *dFile=new OpenFile(sector);
...
FileSystem::Open(char *name)
{
    ...
    for (i = 0; i < folderStrVec.size() - 1; i++) {
	sector=directory->Find((char*)folderStrVec[i].c_str());
	if(sector==-1)
	  return NULL;
	if (currentDirectoryFile != directoryFile)
	  delete currentDirectoryFile;
    ...
    // 洞6:begin
    //   你需要修改并填补这里
    //   根据得到的sector,添加到openedFile里面.(需要判断sector)
...
bool FileSystem::Recover(char *srcName, char *dstName) {
   ...
   // bool success;
    ...
    sector = currentDirectory->Find(srcName,TRUE);
    if(sector==-1)
      return FALSE;
    ...
    char* copy=new char[filelength+10];
    of->Read(copy,filelength);
