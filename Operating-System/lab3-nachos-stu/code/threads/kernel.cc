...
void Kernel::Initialize()
{
    ...
    thread_sch_f = fopen("0schedule_info.txt", "w");
...
{
    pair<map<int, Thread *>::iterator, bool> ret = threadMap.insert(pair<int, Thread *>(tid, thread));
    if (!ret.second)
        cerr << "ERROR when insert thread id=" << tid << " into kernel: it has existed" << endl;
}
void Kernel::removeThread(int tid)
{
    if (threadMap.find(tid) == threadMap.end())
        cerr << "ERROR when remove thread id=" << tid << " from kernel: not exist" << endl;
    else
    {
        threadMap.erase(tid);
        // cout << "remove thread id=" << tid << " from kernel" << endl;
    }
}

Thread *Kernel::getThreadByID(int tid)
{
    // printf("%s getThreadByID id=%d", currentThread->getName(), tid);
    map<int, Thread *>::iterator iter = threadMap.find(tid);
