...
main(int argc, char **argv)
{
    ...
    bool dumpFlag = false;
    char *recoverFileFromName = NULL;
    char *recoverFileToName = NULL;
#endif //FILESYS_STUB
...
for (i = 1; i < argc; i++) {
        ...
        ASSERT(i + 2 < argc);
        recoverFileFromName = argv[i + 1];
        recoverFileToName = argv[i + 2];
        i += 2;
    }
#endif //FILESYS_STUB
    ...
    if (recoverFileFromName != NULL && recoverFileToName != NULL) {
      kernel->fileSystem->Recover(recoverFileFromName,recoverFileToName);
    }
#endif // FILESYS_STUB
