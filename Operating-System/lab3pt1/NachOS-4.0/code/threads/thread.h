...
class Semaphore;
static char ThreadStatusStr[][16] = {
    "JUST_CREATED",
    "RUNNING",
    "READY",
    "BLOCKED"
...
class Thread {
  ...
  public:
    ...
    void AllocTid() { tid=ThreadCnt++;}
    void Join(int tid);
    void AddSP(int tid, Semaphore* s);
    //return 1 if success
    int RemoveSP(int tid);
    Semaphore* GetSP(int tid);
  ...
  private:
    ...
    int tid;
    Thread* parent;
    map<int, Semaphore*> spMap;
    int priority;  //priority for schedule
    /* ++++++++++++++++ LAB2 END +++++++++++++++ */
