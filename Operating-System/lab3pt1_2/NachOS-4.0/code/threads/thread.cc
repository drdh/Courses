...
void
Thread::Join(int tid){
    Thread* child = kernel->getThread(tid);
    if(child==NULL || child->parent!=this){
        //printf("thread %d is not child or has terminated\n",tid);
        return;
    }
    Semaphore* s = new Semaphore("join semaphore",0);
    AddSP(tid,s);
    //printf("tid %d is Ping for %d, get chilid %d\n",kernel->currentThread->tid,tid,child->tid);
    s->P();
    //printf("after P\n");
    RemoveSP(tid);
}
...
void 
Thread::AddSP(int tid, Semaphore* s){
    spMap[tid] = s;
}

int
Thread::RemoveSP(int tid){
    if(spMap[tid]==NULL) return -1;
    delete spMap[tid];
    spMap.erase(tid);
    return 1;
}
...
Thread::Finish()
{
    ...
    if(parent)
    {
        Semaphore* s = parent->GetSP(tid);

        //printf("ving, my parent %d,me %d\n",parent->tid,tid);
        if(s) {
            //printf("can v\n");
            s->V();
        }
